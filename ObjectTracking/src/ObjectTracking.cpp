...
ObjectTracking::ObjectTracking(RTC::Manager* manager)
  ...
  : RTC::DataFlowComponentBase(manager),
    ...
    m_x_outOut("x_out", m_x_out),
    m_y_outOut("y_out", m_y_out)
...
RTC::ReturnCode_t ObjectTracking::onInitialize()
{
  ...
  addOutPort("x_out", m_x_outOut);
  addOutPort("y_out", m_y_outOut);
...
RTC::ReturnCode_t ObjectTracking::onExecute(RTC::UniqueId ec_id)
{
	...
	if (m_orig_imgIn.isNew()){
		...
		m_x_out.data = (int)(trackRegion.center.x - resultImage.size().width / 2);
		m_y_out.data = (int)(trackRegion.center.y - resultImage.size().height / 2);
